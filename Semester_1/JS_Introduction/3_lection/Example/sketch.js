...
function setup() {
  ...
  for (let i = 0; i < 512; i++) amplitudes.push(0);
...
function keyPressed() {
  if (!isInitialised) {
    ...
    if (isLoaded) sound.loop(0, r); // loop - функция для зацикливания. 0 -  откуда начинается зацикливание по времени r - rate - playback rate
  } else {
    if (key == " ") {
