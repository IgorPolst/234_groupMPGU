...
function setup() {
    ...
    menAndWomen = countMenAndWomen(studentData.getColumn('gender'));
    console.log(studentData.getColumn('gender'));
    myBar = new Bar(10, 10, 700, 700);
    myBar.setupAxis(0, 60, 0, 150, 20, 30);
    myBar.addBars(menAndWomen);
}
...
function countMenAndWomen(sours){
    let totalMaleFemale = [{ posX: 20, posY: 0, size: 10, color: "aqua"}, { posX: 40, posY: 0, size: 10, color: "pink" }];
    for(let i = 0; i < sours.length; i++){
        if(sours[i] === 'M') totalMaleFemale[0].posY += 1;
        else totalMaleFemale[1].posY += 1;
    }
...
function Scatter(x, y, width, height) {
    ...
    this.setupAxis = function (
        ...
        for (
            var i = this.plotX;
            i <= this.plotX + this.plotWidth + 1;
            i += scaledXTickSpacing
        ) {
            line(
                i,
                this.plotY + this.plotHeight,
                i,
                this.plotY + this.plotHeight + yPadding * 0.66
            );
        ...
        for (
            var i = this.plotY;
            i < this.plotY + this.plotHeight + 1;
            i += scaledYTickSpacing
        ) {
    ...
    this.lineOfBestFit = function (xValues, yValues, colour) {
        ...
        if (xValues.length == yValues.length) {
            ...
            var m =
                ...
                (xValues.length * sumX2 - sumX * sumX);
...
function Bar(x, y, width, height) {
    ...
    this.setupAxis = function (
        ...
        for (
            ...
            text(xTickValue[count], i, this.plotY + this.plotHeight + yPadding * 0.66);
            count ++;
        }

        //draw the y ticks
        this.scaledYTickSpacing = map(
            yTickSpacing,
            0,
            yMax,
            0,
    ...
    this.addBar = function (xValue, yValue, width, colour) {
        if (
            ...
            var barWidth = (width * this.scaledXTickSpacing) / 10;
            ...
            rect(
                ...
                pointY,
                barWidth,
                this.plotHeight + this.plotY - pointY
            );
        }
    };

    this.addBars = function (mass) {
        for (let i = 0; i < mass.length; i++) {
            myBar.addBar(mass[i].posX, mass[i].posY, mass[i].size, mass[i].color);
        }
    };
}
